...
fu_mtd_recovery_device_to_string(FuDevice *device, guint idt, GString *str)
{
	...
	if (self->gpio_number > 0)
		fu_common_string_append_kx(str, idt, "GpioNumber", self->gpio_number);
	if (self->bind_id != NULL)
		fu_common_string_append_kv(str, idt, "BindId", self->bind_id);
	if (self->kernel_driver != NULL)
		fu_common_string_append_kv(str, idt, "KernelDriver", self->kernel_driver);
}
				    ...
				    GError **error)
{
	...
	if (g_strcmp0(key, "MtdRecoveryGpioNumber") == 0) {
		self->gpio_number = fu_common_strtoull(value);
		return TRUE;
	}
